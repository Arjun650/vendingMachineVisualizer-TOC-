...
async function checkTransaction(product, id) {
  ...
  let ch5 = parseInt(document.getElementById("token5").value);
  let orig5 = parseInt(document.getElementById("token5").value);
  // console.log(token5)
  let token10 = parseInt(document.getElementById("token10").value);
  let ch10 = parseInt(document.getElementById("token10").value);
  let orig10 = parseInt(document.getElementById("token10").value);
  ...
  for (let i = 0; i < need10; i++) {
    ...
    // token10--;
    ch10--;
    document.getElementById("token10").value = ch10
    updateBalance(0,10);
    // if(sum ==0 ) break;
  ...
  for (let i = 0; i < token5; i++) {
    ...
    // token5--; 
    ch5--;
    document.getElementById("token5").value = ch5;
    updateBalance(0,5);
  ...
  if (currentState === product) {
    ...
    document.getElementById("token5").value = orig5
    document.getElementById("token10").value = orig10
    let ammm = orig5 * 5 + orig10 * 10;
    // document.getElementById("balance").innerHTML = "Balance: " + ammm;
    document.getElementById("checkBal").value =ammm;
    currentBalance = ammm
...
let currentBalance = 0// Global variable to store the current balance
// console.log(parseInt(document.getElementById("monn").innerHTML))
function updateBalance(tokenValue , sum) {
  currentBalance += tokenValue;
  if(sum != 0){ 
    currentBalance -= sum; 
  }  
  // Increment the current balance by the token value
  // document.getElementById("monn").innerHTML = "Balance: " + currentBalance;
  // document.getElementById("balance").innerHTML = "Balance: " + currentBalance;
  document.getElementById("checkBal").value =currentBalance;
}

// function checkBalance(){
//   let token5 = parseInt(document.getElementById("token5").value);
//   let token10 = parseInt(document.getElementById("token10").value);
//   document.getElementById("checkBal").value = token5 * 5 + token10 * 10;
// }

function reset() {
  document.getElementById("token5").value = 0;
  document.getElementById("token10").value = 0;
  document.getElementById("checkBal").value = 0;
  currentBalance = 0;
}

function returnMoney(){
  const resultElement = document.getElementById("result");
  let t5 = document.getElementById("token5").value 
  let t10 = document.getElementById("token10").value 
  let total = t5 * 5 + t10 * 10
  currentBalance = 0 ; 
    resultElement.textContent = "Rs" + total + " Returned";
    reset();
    setTimeout(() => {
